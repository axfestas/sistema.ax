...
export default function SweetsPage() {
  ...
  function handleEdit(sweet: Sweet) {
    ...
    setFormData({
      ...
      is_featured: sweet.is_featured ?? 0,
      is_promotion: sweet.is_promotion ?? 0,
      original_price: sweet.original_price ? sweet.original_price.toString() : '',
    });
    setShowForm(true);
  }
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="flex items-center p-3 bg-yellow-50 rounded">
                    <input
                      type="checkbox"
                      id="sweet_is_featured"
                      checked={formData.is_featured === 1}
                      onChange={(e) => setFormData({...formData, is_featured: e.target.checked ? 1 : 0})}
                      className="mr-3 w-4 h-4"
                    />
                    <label htmlFor="sweet_is_featured" className="text-sm font-semibold text-yellow-900 cursor-pointer">
                      ⭐ Em destaque
                    </label>
                  </div>
                  <div className="flex items-center p-3 bg-blue-50 rounded">
                    <input
                      type="checkbox"
                      id="sweet_is_promotion"
                      checked={formData.is_promotion === 1}
                      onChange={(e) => setFormData({...formData, is_promotion: e.target.checked ? 1 : 0})}
                      className="mr-3 w-4 h-4"
                    />
                    <label htmlFor="sweet_is_promotion" className="text-sm font-semibold text-blue-900 cursor-pointer">
                      🏷️ Em promoção
                    </label>
                  </div>
                  <div>
